...
/// Deletes a decrypted file and its associated metadata (if exists).
...
pub(crate) async fn delete_file(
    ...
    if !unlocked_vaults.contains_key(&name) {
        return Err(make_error(StatusCode::FORBIDDEN, "Vault is locked"));
    }
    drop(unlocked_vaults);
