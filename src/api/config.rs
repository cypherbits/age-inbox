...
use crate::inbox_core::{read_vault_config_file, InboxCoreError, VaultConfig};
...
fn map_core_err(err: InboxCoreError) -> ApiError {
    match err {
        ...
        InboxCoreError::InvalidConfig => {
            make_error(StatusCode::INTERNAL_SERVER_ERROR, "Invalid config")
        }
...
pub(crate) async fn read_vault_config(vault_dir: &Path) -> Result<VaultConfig, ApiError> {
    read_vault_config_file(vault_dir)
        .await
